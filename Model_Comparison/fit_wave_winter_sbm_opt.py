...
CSV_FILE = ROOT / "stopkoronavirus_clean_wave_winter.csv"
DATASET_FILE = OUTPUT_DIR / "dataset_normalized.npz"
PLOT_FILE = OUTPUT_DIR / "ajuste_wave_winter_sbm_original.png"
RESULTS_FILE = OUTPUT_DIR / "ajuste_wave_winter_sbm_original.txt"

def load_data():
    df = pd.read_csv(CSV_FILE)
    df["Casos nuevos"] = df["CONFIRMED"]
    return df

def load_dataset_bounds():
    if not DATASET_FILE.exists():
        # Valores de fallback si no existe el npz
        return np.array([0.1, 1.0, 0.6, 4.0]), np.array([0.8, 3.5, 1.2, 40.0])
...
def run_sbm_average(beta_net, beta_hh, delta, fermi_mu, num_sims=10, seed_offset=0):
    ...
    if len(curves) == 0:
        return np.zeros(int(SIMULATION_PARAMS["tmax"])), np.zeros((1, int(SIMULATION_PARAMS["tmax"])))
...
def fit_original_sbm(df, seed, bounds, num_sims, maxiter, maxfev):
    ...
    metrics = {
        ...
        "mse_relativo": mean_squared_error(real_cases, pred_cases) / np.var(real_cases),
...
def save_results(df, params, metrics, pred_cases, lower, upper, seed, bounds, exec_time):
    if "TIME" in df.columns:
        # Extraer solo la fecha de la columna datetime
        dates = pd.to_datetime(df["TIME"]).dt.date.iloc[:len(pred_cases)]
    else:
        dates = np.arange(len(pred_cases))
        
    real_cases = df["Casos nuevos"].to_numpy(dtype=float)[:len(pred_cases)]

    plt.figure(figsize=(13, 7))
    plt.plot(dates, real_cases, color="firebrick", linewidth=2.2, marker="o", markersize=3.5, label="Wave Winter Datos Reales")
    plt.plot(dates, pred_cases, color="darkgreen", linewidth=2.5, label="SBM original ajustado")
    plt.fill_between(dates, lower, upper, color="darkgreen", alpha=0.2, label="Intervalo P10-P90")
    
    plt.title("Ajuste a Ola de Invierno con SBM Mecanístico y Optimización", fontsize=15)
    ...
    text = "\n".join(
        [
            ...
            f"R2 = {metrics['r2']:.4f}",
            f"MSE Rel = {metrics['mse_relativo']:.4f}",
            f"T. Ejec = {exec_time:.2f} s"
        ]
    )
    plt.gca().text(
        0.02,
    ...
    with open(RESULTS_FILE, "w", encoding="utf-8") as f:
        f.write("Ajuste de datos Wave Winter con SBM original\n\n")
        ...
        for key, value in metrics.items():
            f.write(f"{key}: {value:.10g}\n")
        f.write(f"Tiempo de ejecución: {exec_time:.2f} s\n")
        f.write(f"\nGrafica: {PLOT_FILE}\n")

def parse_args():
    parser = argparse.ArgumentParser(description="Ajusta el SBM original a datos Wave Winter.")
    parser.add_argument("--num-sims", type=int, default=10, help="Numero de replicas SBM por evaluacion.")
    parser.add_argument("--maxiter", type=int, default=20, help="Iteraciones maximas de Powell.")
    parser.add_argument("--maxfev", type=int, default=60, help="Evaluaciones maximas de la funcion objetivo.")
...
def main():
    ...
    # Valores de la semilla provista por el ajuste del LSTM surrogate
    seed = {
        "beta_net": 0.2143,
        "beta_hh": 2.4335,
        "delta": 1.1722,
        "fermi_mu": 22.4418,
        "scale_cases": 1724663.41,
        "shift_days": 4.0
    }
    ...
    print("Semilla surrogate proporcionada:")
    for key, value in seed.items():
        print(f"  {key}: {value:.6g}")
    print("Bounds de busqueda (espacio local):")
    for name, (lo, hi) in zip(["beta_net", "beta_hh", "delta", "fermi_mu", "shift_days"], bounds):
    ...
    save_results(df, params, metrics, pred_cases, lower, upper, seed, bounds, exec_time)

    print("\nAjuste SBM original completado.")
